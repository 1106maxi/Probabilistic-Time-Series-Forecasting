...
for rep in range(1):
    for time in time_steps:
        for batch in batch_size:
                    ...
                    QLSTM_opt.rolling_forecast_v(loss= QLSTM_opt.sym_loss, model= QLSTM_opt.lstm_model_simpler)
...
for rep in range(9):
    for time in time_steps:
        for batch in batch_size:
                    QLSTM_opt = qlstm("UPS",start_date="2009-01-01",end_date= "2011-09-01",time_steps =time, batch_size = batch,forecasting_window = 10)
                    QLSTM_opt.rolling_forecast_v(loss= QLSTM_opt.sym_loss, model= QLSTM_opt.lstm_model_complex)
                    result = QLSTM_opt.predict_eval()
